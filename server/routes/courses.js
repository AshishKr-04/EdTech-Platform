...
// 👉 Course Student Roster with progress tracking
router.get(
  "/instructor/courses/:id/roster",
  authMiddleware,
  restrictTo("Teacher", "Admin"),
  async (req, res, next) => {
    try {
      ...
      if (course.instructor.toString() !== req.user.id && req.user.role !== "Admin") {
        return next(new AppError("You are not authorized to view the roster of this course", 403));
      }

      const students = await User.find({
        enrolledCourses: req.params.id,
      }).select("name email lessonsCompleted createdAt");

      const totalLessons = course.lessons.length;

      const roster = students.map((student) => {
        const completedCount = student.lessonsCompleted.filter(
          (lc) => lc.courseId && lc.courseId.toString() === course._id.toString()
        ).length;
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const isInstructor = course.instructor._id.toString() === req.user.id || req.user.role === "Admin";
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await User.findById(req.user.id);
      if (!user.enrolledCourses.includes(id) && course.instructor.toString() !== req.user.id && req.user.role !== "Admin") {
