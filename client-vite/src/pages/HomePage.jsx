...
const HomePage = () => {
    ...
    const fetchHomeData = async () => {
      try {
        ...
        if (auth.user?.role === "Instructor") {
          const [ownCourses, publicCourses] = await Promise.all([
            api.get("/courses/instructor/my-courses"),
            api.get("/courses"),
          ]);
          setInstructorCourses(ownCourses.data.courses || []);
          setCourses(publicCourses.data.courses || []);
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-[#070a13] text-slate-100 antialiased font-sans selection:bg-indigo-500/30 transition-colors duration-300">
            ...
            <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-slate-400">
                  {auth.user?.role === "Instructor" ? "Instructor Workspace" : "Student Workspace"}
                </p>
                ...
                <p className="mt-1 text-sm text-slate-350">
                  {auth.user?.role === "Instructor"
                    ? `You have ${instructorCourses.length} course${instructorCourses.length === 1 ? "" : "s"} in your instructor workspace.`
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Link
                  to={auth.user?.role === "Instructor" ? "/instructor-dashboard" : "/my-courses"}
                ...
                {auth.user?.role === "Instructor" && (
                  <Link
                    to="/create-course"
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-650 bg-indigo-600 hover:bg-indigo-500 px-4 py-2.5 text-sm font-bold text-white transition shadow-sm"
                  >
                    Create Course
                  </Link>
                )}
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-8 rounded-xl border border-slate-850 bg-[#0e1326] p-8 text-white shadow-2xl lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div className="text-left">
            <p className="text-xs font-bold uppercase tracking-wider text-indigo-300">
              Teach on EduMind
            </p>
            <h2 className="mt-2 text-3xl font-black tracking-tight">
              Create courses, upload lessons, and track learner engagement.
            </h2>
            <p className="mt-3 text-sm leading-6 text-slate-350">
              The instructor side includes course publishing, lesson management,
              video upload support, and analytics for enrollments.
            </p>
            <Link
              to={auth.isAuthenticated ? "/create-course" : "/register"}
              className="mt-6 inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 px-5 py-3 text-sm font-bold text-white transition shadow-lg shadow-indigo-600/20"
            >
              Start Teaching
              <ArrowRight className="h-4 w-4" />
            </Link>
          ...
          <div className="grid gap-3">
            {[
              "Publish courses with lessons and media",
              "Manage your created courses",
              "Track total students and course performance",
              "Edit course content from the dashboard",
            ].map((item) => (
              <div
                key={item}
                className="flex items-center gap-3 rounded-lg border border-slate-850 bg-[#070913]/60 p-3"
              >
