        ...
        <div className="md:col-span-5 bg-slate-50 border-r border-slate-200 text-slate-800 p-10 flex flex-col justify-between hidden md:flex relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            <div className="flex items-center gap-2">
              <span className="text-xl">{selectedRole === 'Student' ? '🎓' : '👨‍🏫'}</span>
            ...
            <div className="space-y-4 pt-12 text-left">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                {selectedRole === 'Student'
                  ...
                  : 'Publish educational syllabus modules, monitor student enrollment analytics, and issue cryptographically verifiable graduation certificates.'}
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 border-t border-slate-200 pt-6 text-left">
            <p className="text-[10px] text-slate-500 font-mono">
              {selectedRole === 'Student' ? 'EduMind Student Auth Node' : 'EduMind Instructor Auth Node'}
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-7 p-8 md:p-10 flex flex-col justify-center bg-white">
          ...
          <div className="mb-6 text-left">
            <div className="bg-slate-100 w-12 h-12 rounded-xl flex items-center justify-center mb-3 text-slate-800">
              {selectedRole === 'Student' ? <GraduationCap size={22} /> : <BookOpen size={22} />}
            </div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">
              {selectedRole === 'Student' ? 'Student Portal' : 'Instructor Portal'}
            </h1>
            <p className="text-xs text-slate-500 mt-1">
              Sign in to your {selectedRole === 'Student' ? 'learning dashboard' : 'instructor console'}
            </p>
          </div>

          {/* Role Portal Segmented Selector Toggle */}
